...
将自然语言需求转换为面向 FPGA 的可综合 Verilog-2005 RTL。优先保证硬件行为正确、综合安全、接口清晰。
...
6. **注释使用中文**
   - 所有 Verilog 代码注释须使用中文；专用术语、变量名、参数名、公式符号、端口名等可保留英文
   - 例如：`// 数据总线位宽` 而非 `// Data bus width in bits`；但 `DATA_WIDTH`、`AXI`、`FIFO` 等术语保留英文
...
生成代码后运行自检，`[FAIL]` 项修复后再输出。完整自检清单见 `references/04_review_checklist.md`。
